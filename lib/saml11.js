...
function extractSaml11Options(opts) {
  return {
    ...
    microsoftMfa: opts.microsoftMfa
  };
}
...
function createAssertion(options, strategies, callback) {
  ...
  const authenticationMethod = options.microsoftMfa ? 'urn:oasis:names:tc:SAML:1.0:am:password' : 'http://schemas.microsoft.com/claims/multipleauthn'
  doc.getElementsByTagName('saml:AuthenticationStatement')[0]
    .setAttribute('AuthenticationMethod', authenticationMethod);
